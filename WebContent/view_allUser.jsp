...
%>
	...
	<div class='offset-sm-3 col-sm-6 mt-5 mb-5'>
		<form action='view_searchUser.jsp' method='post'>
			<div class='form-group'>
				<div class="row">
					<input type='text' placeholder="漢字で入力をお願いします"
						class='form-control offset-3 col-5' name='query' required>
					<button type='submit' class='btn btn-primary col-3'>検索</button>
				</div>
			</div>
		</form>
	</div>
				...
				<div class="row">
					<div class="col-8">
						<form action='view_friendTaking.jsp' method='post'>
							<input type='hidden' name="user_id" value='<%=user.getUser_id(i)%>'>
							<div class="row">
					...
					if (user.getFavo_list().contains(user.getUser_id(i))){%>
						...
						<input type="hidden" name="user_favo_to" value="<%=user.getUser_id(i) %>">
	...
	<%
		if (num == 0) {
	%>
	<p class="text-center">あなた以外に友達はいないようです・・・</p>
	<%}%>
